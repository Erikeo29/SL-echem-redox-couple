...
- **Étude 1** CV couple redox sur Au : le couple redox de type ferro/ferricyanure en solution, résolu par éléments finis, illustre le diagnostic de réversibilité et l'influence de $k^0$, $\alpha$ et $\nu$ sur la forme du voltammogramme. Les tendances observées (loi de Randles-Ševčík, transition irréversible → réversible) sont cohérentes avec la théorie.
...
Ces travaux constituent un point de départ pour des études plus approfondies, et non une caractérisation définitive des systèmes considérés. Une validation sur la base d'une comparaison avec des résultats expérimentaux reste nécessaire.
...
- **PINNs (Physics-Informed Neural Networks)** : entraîner un réseau de neurones contraint par les équations de Butler-Volmer et de diffusion pour accélérer la résolution.
